...
def encrypt12(db_file, key_file, db_cript, output):
    ...
    try:
        ...
        print(f"[-] {db_file} encrypted, '{output}' created")

    except Exception as e:
        print(f"[e] An error has ocurred encrypting '{db_file}' - ", e)
...
def decrypt14(db_file, key_file, path, offset):
    ...
    try:
        ...
        print(f"[-] {db_file} decrypted, '{path}' created")
        return True

    except Exception as e:
        print(f"[e] An error has ocurred decrypting '{db_file}' - ", e)
        return False
        ...
        if args.encrypt:
            if os.path.exists(args.file):
                if os.path.exists(args.encrypt[0]) and os.path.exists(args.encrypt[1]):
                    ...
                    print(f"[e] '{args.encrypt[0]}' or '{args.encrypt[1]}' doesn't exist")
            else:
                print(f"[e] {args.file} doesn't exist")

        elif args.decrypt:
            if os.path.exists(args.file):
                if os.path.exists(args.decrypt):
                    print("[i] Starting to decrypt...")
                    if os.path.splitext(args.file)[1] == ".crypt12":
                        decrypt12(args.file, args.decrypt, args.output)

                    elif os.path.splitext(args.file)[1] == ".crypt14":
                        for offset in range(185, 195):
                            if decrypt14(args.file, args.decrypt, args.output, offset):
                                break
                else:
                    print(f"[e] {args.decrypt} doesn't exist")
            else:
                print(f"[e] {args.file} doesn't exist")
